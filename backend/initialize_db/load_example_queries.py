...
async def load_examples(json_file):
    with open(json_file) as f:
        examples = json.load(f)
        for ex in examples:
            ex = ExampleQuery.parse_obj(ex)
            result = await snap_write(ex)

if __name__ == "__main__":
    assert len(sys.argv) > 1 and len(sys.argv) < 3, '\n\nload_example_queries example usage:\n\npython load_example_queries.py example_queries.json\n'

    loop = asyncio.get_event_loop()
    work = [loop.create_task(load_examples(sys.argv[1]))]
